...
* __클러스터링 기반 이상감지 모델__
    ...
    * __LSTM-AutoEncoder 이상감지 모델__
        ...
        * Autoencoder 방식 설명
        * 지도학습 방식의 Neural Network는 데이터의 입력값과 목표값(target)이 모두 주어진 라벨링된 데이터를 모델 학습에 사용합니다. 반대로 비지도학습 방식은 데이터의 입력값만 주어진 상태로 모델을 학습합니다. Auto-Encoder Neural Network(AutoEncoder, 오토인코더)는 비지도학습 방식입니다. 오토인코더는  
